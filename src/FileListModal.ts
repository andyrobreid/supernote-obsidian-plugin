...
export abstract class FileListModal extends SuggestModal<SupernoteFile> {
	...
	async loadFiles(): Promise<void> {
		try {
			const response = await fetch(
				`http://${this.settings.directConnectIP}:8089${this.currentPath}`,
			);
			if (!response.ok) {
			...
			// Connection failed - try to rediscover the device
			const rediscovered = await this.tryRediscoverDevice();
			if (rediscovered) {
				// Retry loading files with the new IP
	...
	async tryRediscoverDevice(): Promise<boolean> {
		const notice = new Notice(
			'Connection failed. Attempting to rediscover device...',
			0,
		);

		try {
			// First, try to verify if the old IP still works
			if (this.settings.directConnectIP) {
				const stillValid = await verifySupernoteDevice(
					this.settings.directConnectIP,
				);
				if (stillValid) {
					notice.hide();
					return false; // IP is still valid, the error was something else
				}
			}

			// Extract subnet from old IP to search the same network
			const subnet = this.settings.directConnectIP
				? getSubnetFromIP(this.settings.directConnectIP)
				: null;

			if (!subnet) {
				notice.hide();
				new Notice(
					'Cannot determine network subnet. Please configure IP manually.',
				);
				return false;
			}

			// Old IP doesn't work, scan for devices on the same subnet
			const devices = await discoverSupernoteDevices(subnet);
			notice.hide();

			if (devices.length === 0) {
				new Notice(`Could not find any Supernote devices on ${subnet}.x`);
				return false;
			} else if (devices.length === 1) {
				// Auto-select the only device found
				this.settings.directConnectIP = devices[0].ip;
				await this.plugin.saveSettings();
				new Notice(
					`Reconnected to ${devices[0].deviceName} at ${devices[0].ip}`,
				);
				return true;
			} else {
				// Multiple devices found - use the first one (fastest response)
				this.settings.directConnectIP = devices[0].ip;
				await this.plugin.saveSettings();
				new Notice(
					`Reconnected to ${devices[0].deviceName} at ${devices[0].ip}`,
			...
			notice.hide();
			new Notice(`Rediscovery failed: ${err.message}`);
			return false;
		}
	}
		...
		if (file.isDirectory) {
			...
			try {
				const fileResponse = await fetch(
					`http://${this.settings.directConnectIP}:8089${file.uri}`,
				);
				if (!fileResponse.ok) {
					throw new Error(
				...
				const filename =
					await this.app.fileManager.getAvailablePathForAttachment(file.name);
